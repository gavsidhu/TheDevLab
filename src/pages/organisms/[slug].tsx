...
import OrganismLayout from '@/components/organisms/OrganismLayout';
...
const Molecule = ({ source, headings, post }: PostProps) => {
    return (
        <OrganismLayout source={source} headings={headings} post={post} />
    )
}
...
export const getStaticPaths: GetStaticPaths = async () => {
    ...
    const paths = contentData
        .filter((item) => item.slug.startsWith('organisms/'))
        .map(({ slug }) => {
            const organismsSlug = slug.replace('organisms/', '');
            const params = { slug: organismsSlug };
            return { params };
        });
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
    ...
    const { slug } = params;
    const contentData = await getMdxContent();
    const post = contentData.find((item) => item.slug === `organisms/${slug}`);
